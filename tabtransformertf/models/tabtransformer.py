...
import math
from tabtransformertf.utils.helper import build_mlp, build_prompts
...
class TabTransformer(tf.keras.Model):
    def __init__(
        ...
        self.prompts = build_prompts(units, mlp_hidden_factors)
        # Initialise encoder
        if encoder:
    ...
    def call(self, inputs):
        ...
        for i in range(len(self.mlp_final)):
            if i % 3 == 1:
                x1 = self.mlp_final[i](x)
                x2 = self.prompts[i // 3](x)
                x = tf.keras.layers.concatenate([x1, x2], axis=-1)
            else:
                x = self.mlp_final[i](x)
...
class LoraLayer(Layer):
    def __init__(
        ...
        self.units = original_layer.get_layer(index=2).units
        self.rank = rank
        self.scale = alpha / self.rank

        down_initializer = tf.keras.initializers.RandomNormal(stddev=1 / self.rank)
        up_initializer = tf.keras.initializers.Zeros()

        self.lora_down = Dense(self.rank, kernel_initializer=down_initializer)
        self.lora_up = Dense(self.units, kernel_initializer=up_initializer)

    def call(self, inputs):
        original_output = self.original_layer(inputs)
        lora_output = self.lora_up(self.lora_down(inputs)) * self.scale
        return original_output + lora_output


class AdapterLayer(Layer):
    def __init__(self, pretrained_ffn):
        super(AdapterLayer, self).__init__()
        self.pretrained_ffn = pretrained_ffn

        self.down_project = Dense(
            4,
        ...
        self.up_project = Dense(
            self.pretrained_ffn.get_layer(index=2).units,
            kernel_initializer=tf.keras.initializers.TruncatedNormal(stddev=1e-3),
            bias_initializer="zeros",
    ...
    def call(self, inputs):
        ...
        output_1 = self.pretrained_ffn(inputs)
        output_2 = self.down_project(output_1)
        output_3 = self.up_project(output_2)
        return output_3 + output_1
