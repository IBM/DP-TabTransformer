...
def build_mlp(input_dim, factors, dropout):
    hidden_units = [int(input_dim / f) for f in factors]
    mlp_layers = []
    for units in hidden_units:
        mlp_layers.append(BatchNormalization()),
        mlp_layers.append(Dense(units,
                                activation=selu,
...
def build_prompts(units, factors):
    prompt_layers = [Dense(units, activation=selu,kernel_initializer = tf.keras.initializers.Zeros()) for _ in range(len(factors))]
